...
const PERMISSOES = {
  admin: {
    ...
    verFolha: true, editarFolha: true,
    verSalarios: true,
  ...
  direcao: {
    ...
    verFolha: true, editarFolha: false,
    verSalarios: true,
  ...
  coordenador: {
    verColaboradores: false, editarColaboradores: false,
    verProfessores: true, editarProfessores: false,
    verFolha: false, editarFolha: false,
    verSalarios: false,
    verAcademico: true, editarAcademico: true,
    verRelatorios: true,
    verHistorico: false,
    gerenciarUsuarios: false,
    verTodosCursos: false, // Apenas cursos do coordenador
  },
}
...
export function PermissaoProvider({ children }) {
  ...
  useEffect(() => {
    ...
    supabase.from('perfis_usuario')
      ...
      .single()
      .then(({ data }) => {
        setPerfil(data || { perfil: 'rh', cursos_permitidos: null }) // padrão RH
